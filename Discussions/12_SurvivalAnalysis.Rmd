...
```{r CPH Linearity2, fig.height=6}
gbsg$.martingale <- residuals(model1)
...
plot_grid(
  ...
  ggplot(gbsg, aes(lnodes, .martingale)) +
    geom_jitter() + 
    geom_smooth(method = 'loess', linetype = 2, se = FALSE),
...
\scriptsize
```{r update2}
(model2 <- coxph(surv ~ size + (grade==1) + lnodes + ler, data = gbsg)) %>%
  summary()
```
...
Probably will stick with untransformed er$\ldots$ or perhaps just drop it altogether.

```{r proportionality3, echo=TRUE}
cox.zph(model2)
```
...
\scriptsize
```{r update3}
(model3 <- coxph(surv ~ size + (grade==1) + lnodes, data = gbsg)) %>%
  summary()
```
...
In some instances, we have variables that change over time. In the table below, individual 5 has several creatinine measurements over a 12 month period prior to death,
as well as a disease related infection toward the end.
...
```{r future plot}
plot(model, ylab = 'Survival', xlab = 'Years')
```
